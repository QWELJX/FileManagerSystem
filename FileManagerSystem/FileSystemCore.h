...
fs::path makeAbsoluteFromCurrentDir(
    const std::string &relative_or_abs); // 基于系统exe当前目录
fs::path makeAbsolutePath(const std::string &path,
...
// 字符处理
std::pair<int, int> parseUtf8Char(const unsigned char *str, size_t len,
                                  size_t pos);
// 内部实现细节
...
private:
  ...
  // 文件夹递归输出优化相关函数
  size_t estimateDirectorySize(const fs::path &dir) const; // 估算目录规模
  void printTreeRecursive(const fs::path &dir, std::ostringstream &output, std::vector<bool> is_last_stack,int depth=0) const; // 递归树状输出
  void printTreeIterator(const fs::path &dir,std::ostringstream &output) const; // 迭代器树状输出
  std::string generateTreePrefix(const std::vector<bool> &is_last_file_appeared,int depth) const; // 生成树形前缀
};