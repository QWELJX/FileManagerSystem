...
bool FileManager::handleMove(std::string oldPath, std::string newPath) {
	oldPath = PathUtils::uniformSeparator(oldPath);
	newPath = PathUtils::uniformSeparator(newPath);
    oldPath = getAbsolutePath(oldPath);
    newPath = getAbsolutePath(newPath);
    TreeNode* node = FindNodeByPath(oldPath);
    DirectoryNode* newDirectoryNode = dynamic_cast<DirectoryNode*>(FindNodeByPath(newPath));
    ...
    if (newDirectoryNode->GetType() != TreeNodeType::DIRECTORY) {
        // 获取目标文件的目录
...
//
//    // 更新当前节点的路径
//    node->SetPath(newPath+node->GetName);
...
void FileManager::updateAllChildPaths(TreeNode* node, const std::string& newPath) {
    ...
    // 拼接当前节点的新路径（用工具函数保证分隔符正确）
    std::string currentNewPath = PathUtils::join(newPath, node->GetName());
    // 更新当前节点路径
    node->SetPath(currentNewPath);

    // 同步更新路径映射表（先删旧，再加新）
    RemoveNodeFromPathMap(oldPath);
    SetNodeInPathMap(currentNewPath, node);
    ...
    if (dirNode != nullptr) {
        ...
        for (TreeNode* child : children) {
            // 构建子节点新路径
            std::string childNewPath = PathUtils::join(currentNewPath, child->GetName());
            updateAllChildPaths(child, childNewPath);
        }
    }
}
