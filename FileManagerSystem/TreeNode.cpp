...
// 删除指定的子节点（仅保留 TreeNode* 类型参数，无多余头文件/声明）
void DirectoryNode::DelChild(TreeNode* child) {
    // 1. 安全检查：避免空指针操作
    if (child == nullptr) {
        return;
    }

    // 2. 从 FileManager 的路径映射中移除该节点
    FileManager::getInstance().RemoveNodeFromPathMap(child->GetPath());

    // 3. 从类成员 children 列表中删除该子节点（erase-remove 惯用法）
    auto it = std::remove(children.begin(), children.end(), child);
    if (it != children.end()) {
        // 移除列表中匹配的节点
        children.erase(it, children.end());

        // 可选：释放节点内存（根据你的内存管理策略决定是否启用）
        // delete *it;
        // *it = nullptr;
    }
}

// FileManager 类的 RemoveNodeFromPathMap 方法纯实现
void FileManager::RemoveNodeFromPathMap(const std::string& path) {
    // 查找并删除指定路径的映射项
    auto mapIt = pathMap.find(path);
    if (mapIt != pathMap.end()) {
        pathMap.erase(mapIt);
    }
   mapIt = pathMap.find(path+SEPARATOR);
    if (mapIt != pathMap.end()) {
        pathMap.erase(mapIt);
    }
}
