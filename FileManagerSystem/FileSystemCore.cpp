...
// 改进的格式化函数：可指定对齐方式
std::string FileSystemUtils::padToDisplayWidth(const std::string &str,
                                               int targetWidth,
                                               bool leftAlign = true) {
...
// 辅助函数：基于系统当前目录生成绝对路径
fs::path FileSystemUtils::makeAbsoluteFromCurrentDir(
    const std::string &relative_or_abs) {
...
fs::path FileSystemUtils::makeAbsolutePath(const std::string &path,
                                           const fs::path &base) {
  // 将UTF-8字符串转换为宽字符串，确保中文等非ASCII字符正确处理
...
bool FileSystemCore::deleteSingle(const fs::path &target) {
  try {
    ...
    if (ec) {
      setLastError(
          "删除失败: " + FileSystemUtils::WideToUTF8(target.wstring()) + " - " +
          ec.message());
      return false;
    }
...
// 移动/复制文件或目录
bool FileSystemCore::movePath(const fs::path &source,
...
// 重命名
bool FileSystemCore::renamePath(const fs::path &old_name,
...
void FileSystemCore::listDirectoryDetail(const fs::path &dir,
  ...
  try {
    ...
    std::string header_name =
        FileSystemUtils::padToDisplayWidth("Name", NAME_WIDTH);
    std::string header_type =
        FileSystemUtils::padToDisplayWidth("类型", TYPE_WIDTH);
    std::string header_time =
        FileSystemUtils::padToDisplayWidth("修改时间", TIME_WIDTH);
    std::string header_size =
        FileSystemUtils::padToDisplayWidth("大小", SIZE_WIDTH, false); // 右对齐
    ...
    for (const auto &entry : entries) {
      try {
        ...
        if (is_dir) {
          ...
          std::string ext =
              FileSystemUtils::WideToUTF8(entry.path().extension().wstring());
        ...
        std::string display_name =
            FileSystemUtils::padToDisplayWidth(name, NAME_WIDTH, true);
        // 时间列：左对齐（纯ASCII，但为了统一也格式化）
        std::string display_time =
            FileSystemUtils::padToDisplayWidth(time_buf, TIME_WIDTH, true);
        // 类型列：左对齐
        std::string display_type =
            FileSystemUtils::padToDisplayWidth(type_name, TYPE_WIDTH, true);
        // 大小列：右对齐（数字通常右对齐）
        std::string display_size =
            FileSystemUtils::padToDisplayWidth(size_str, SIZE_WIDTH, false);

        // 输出格式化后的行，手动控制列间距
        output << display_name << std::string(COL_SPACING, ' ') << display_type
               << std::string(COL_SPACING, ' ') << display_time
               << std::string(COL_SPACING, ' ') << display_size << "\n";
...
void FileSystemCore::listDirectoryRecursive(const fs::path &dir,
                                            std::ostringstream &output) const {
  try {
	  if (estimateDirectorySize(dir) >10000) {
		  printTreeIterator(dir, output);
          return;
      }
      else {
		  printTreeRecursive(dir, output, std::vector<bool>(1,false), 1);
          return;
      }
...
void FileSystemCore::listDirectoryImpl1(const fs::path &dir,
                                        std::ostringstream &output) const {
  try {
    output << "目录: " << FileSystemUtils::WideToUTF8(dir.wstring()) << "\n\n";
...
#pragma region 文件夹递归输出优化实现

// 估算目录规模：返回目录的总条目数（文件+文件夹）
size_t FileSystemCore::estimateDirectorySize(const fs::path &dir) const {
  size_t count = 0;
  try {
    // 使用迭代器快速估算，避免递归遍历
    for (fs::recursive_directory_iterator it(dir, fs::directory_options::skip_permission_denied),end;it != end; ++it) {
      ++count;
      // 限制最大估算数量，避免在超大目录上耗时过长
      if (count > 10000) {
        break;
      }
    }
...
// 生成树形前缀：根据深度和是否为最后条目生成合适的树形连接符
std::string FileSystemCore::generateTreePrefix(const std::vector<bool> &is_last_file_appeared,int depth) const {
  if (depth <= 0) {
    return "  "; // 根目录或无效深度没有前缀
  }
  ...
  for (int i = 0; i < depth-1; ++i) {
	  ...
	  prefix += is_last_file_appeared[i] ? FileSystemUtils::padToDisplayWidth(" ", 5) : FileSystemUtils::padToDisplayWidth("│ ", 6);
  }

  return prefix;
}

// 递归树状输出：生成完美的树状结构，支持排序和性能优化
void FileSystemCore::printTreeRecursive(const fs::path &dir,
                                        std::ostringstream &output,
                                        std::vector<bool> is_last_file_appeared,
                                        int depth) const {
  try {
    // 收集当前目录的所有条目，预分配内存
    ...
    try {
      ...
      estimated_size = 16; // 默认大小
    }

    children.reserve(estimated_size);
    ...
    // 智能排序：小目录立即排序，大目录延迟排序或不排序
    const size_t SORT_THRESHOLD = 100; // 小于100个条目时才排序
    if (children.size() <= SORT_THRESHOLD&&children.size()>1) {
      std::sort(children.begin(), children.end(),
                [](const fs::directory_entry &a, const fs::directory_entry &b) {
                  bool a_is_dir = a.is_directory();
                  bool b_is_dir = b.is_directory();
                  if (a_is_dir != b_is_dir) {
                    return a_is_dir > b_is_dir; // 目录在前
                  }
                  return a.path().filename().string() <
                         b.path().filename().string();
                });
    ...
    //  std::string dir_name =
    //      FileSystemUtils::WideToUTF8(dir.filename().wstring());
    //  if (dir_name.empty()) { // 根目录
    //    dir_name = FileSystemUtils::WideToUTF8(dir.wstring());
    //  }
    //  output << current_prefix << "[+] " << dir_name << "/\n";
    //}

    // 处理每个条目
    for (size_t i = 0; i < children.size(); ++i) {
      const auto &entry = children[i];
      bool is_last_entry = (i == children.size() - 1);
      std::string name =FileSystemUtils::WideToUTF8(entry.path().filename().wstring());

      // 生成父级缩进前缀
      std::string entry_prefix =generateTreePrefix(is_last_file_appeared,depth);
...
//      }
      if (is_last_entry) {
          entry_prefix +="└── ";
		  is_last_file_appeared[depth - 1] = true;
      }
      else {
          entry_prefix += "├── ";
      }
      ...
      try {
        if (entry.is_directory()) {
          // 输出目录
          output << entry_prefix <<"[+]"<< name << "/\n";

          // 递归处理子目录
//          if (max_depth < 0 || max_depth > 0) {
//            std::vector<bool> child_stack = is_last_file_appeared;
//            child_stack.push_back(is_last_entry);
//            printTreeRecursive(entry.path(), output, "", child_stack,
//                               max_depth > 0 ? max_depth - 1 : max_depth);
//          }
          is_last_file_appeared.push_back(is_last_entry);
          printTreeRecursive(entry.path(), output,is_last_file_appeared,depth + 1);
        } else {
          // 输出文件
          output << entry_prefix <<"[-]"<< name << "\n";
        }
        ...
        // 无法访问的条目
        output << entry_prefix <<"[?]"<< name << " [无法访问]\n";
      }
    }
  } catch (const fs::filesystem_error &e) {
    output << "[错误] 无法访问目录: " << e.what() << "\n";
  }
}

// 迭代器树状输出：适用于大规模目录，内存效率高
void FileSystemCore::printTreeIterator(const fs::path &dir,std::ostringstream &output) const {
  try {
    output << "目录: " << FileSystemUtils::WideToUTF8(dir.wstring()) << "\n\n";
    // 使用递归目录迭代器
    for (fs::recursive_directory_iterator it(dir, fs::directory_options::skip_permission_denied),end;it != end; ++it) {
      try {
        ...
        // 更新is_last_flags（这里简化处理，实际很难准确判断）
        // 这是迭代器方法的局限性
      } catch (const fs::filesystem_error &) {
        int depth = it.depth();
        std::string prefix(depth * 2, ' ');
        output << prefix << "[?] 无法访问的条目\n";
      }
    }
  } catch (const fs::filesystem_error &e) {
    output << "无法开始遍历: " << e.what() << "\n";
  }
}
